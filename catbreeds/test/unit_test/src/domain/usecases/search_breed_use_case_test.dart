...
import 'search_breed_use_case_test.mocks.dart';

@GenerateMocks([BreedsRepository])
void main() {
  late SearchBreedUseCase useCase;
  ...
  setUp(() {
    ...
    useCase = SearchBreedUseCase(repository: mockRepository);
  });

  group('GetCatBreedsUseCase', () {
    test('should call getCatBreeds method on breeds repository', () async {
      //arrange
      when(mockRepository.searchBreed('Aby')).thenAnswer((_) async => const Right([breed]));
      //act
      final response = await useCase('Aby');
      //assert
      verify(mockRepository.searchBreed('Aby')).called(1);
    ...
    test('should return Failure when the call getCatBreeds method on breeds repository fails', () async {
      //arrange
      when(mockRepository.searchBreed('Aby')).thenAnswer((_) async => const Left(ServerFailure()));
      //act
      final response = await useCase('Aby');
      //assert
      verify(mockRepository.searchBreed('Aby')).called(1);
